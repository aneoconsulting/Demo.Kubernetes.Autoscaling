...
resource "kubernetes_service" "application" {
  metadata {
    ...
    labels    = {
      ...
      service = kubernetes_deployment.application.metadata.0.labels.service
    }
  }
  spec {
    ...
    port {
      name        = kubernetes_deployment.application.spec.0.template.0.spec.0.container.0.port.0.name
      port        = var.port
