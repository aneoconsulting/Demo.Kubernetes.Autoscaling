...
locals {
  ...
  ima_aws_node_termination_handler_name                = "${var.cluster_name}-aws-node-termination-handler-${random_string.random_resources.result}"
  aws_node_termination_handler_asg_name                = "${var.cluster_name}-asg-termination"
  aws_node_termination_handler_spot_name               = "${var.cluster_name}-spot-termination"

  # Custom ENI
  subnets = {
    subnets = [
    for val in var.vpc.availability_zones : {
      subnet_id          = val.subnet_id
      az_name            = val.availability_zone
